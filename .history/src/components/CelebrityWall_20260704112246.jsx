...
import kureshi from "../assets/kureshi.jpg";
import krish from "../assets/krish.jpg";
import bhavana from "../assets/bhavana.jpg";
import parvathy from "../assets/parvathy.jpeg";
import kr from "../assets/krr.jpeg";
...
// First 2 anchors sit in the top row beside the text.
// Remaining 8 flow into a 4-per-row grid below.
// TODO: replace image imports below with actual anchor photos as you add them.
const topAnchors = [
  {
    name: "Bavana",
    role: "Celebrity Anchor",
    image: bhavana,
    accent: "#c9a03d",
  },
  {
    name: "Azar",
    role: "Event Host & Anchor",
    image: anchor,
    accent: "#d4af37",
  },
];
...
const CelebrityWall = () => {
  ...
  const [hoveredAnchorIndex, setHoveredAnchorIndex] = useState(null);
  ...
  return (
    <section className="cw-premium-section" ref={sectionRef}>
      ...
      <div className="cw-premium-container">
        ...
        {/* ===========================
            ANCHORS — ROW 1 (text + 2 cards)
        =========================== */}

        <div className="cw-anchor-section">
          {/* Left Text */}
          <div className="cw-anchor-content">
            <span className="cw-anchor-tag">PERFORMANCE EXCELLENCE</span>
            ...
            <p className="cw-anchor-description">
              Charismatic artists who brought energy, elegance, and warmth to
              every celebration.
            </p>
          ...
          {/* Top Row Anchor Cards */}
          {topAnchors.map((anchorItem, index) => (
            <div
              key={index}
              className={`cw-premium-card cw-anchor-card ${
                hoveredAnchorIndex === index ? "hovered" : ""
              }`}
            ...
            >
              ...
              <div className="cw-premium-card-image">
                <img src={anchorItem.image} alt={anchorItem.name} />
              ...
              <div className="cw-premium-card-info">
                <span className="cw-card-label">FEATURED ANCHOR</span>

                <h3 className="cw-premium-card-name">{anchorItem.name}</h3>

                <p className="cw-premium-card-role">{anchorItem.role}</p>

                <div
                  className="cw-premium-card-accent"
                  style={{
                    background: `linear-gradient(90deg, ${anchorItem.accent}, transparent)`,
                  }}
                ></div>
              </div>
            </div>
        ...
        {/* ===========================
            ANCHORS — REMAINING ROWS (4 per row)
        =========================== */}

        <div className="cw-anchor-grid-rest">
          {restAnchors.map((anchorItem, index) => {
            const globalIndex = index + topAnchors.length;
            return (
              <div
                key={globalIndex}
                className={`cw-premium-card cw-anchor-card ${
                  hoveredAnchorIndex === globalIndex ? "hovered" : ""
                }`}
                onMouseEnter={() => setHoveredAnchorIndex(globalIndex)}
                onMouseLeave={() => setHoveredAnchorIndex(null)}
              >
                <div className="cw-card-border"></div>
                ...
                <div className="cw-premium-card-info">
                  <span className="cw-card-label">FEATURED A</span>
          ...
          <div className="cw-premium-statement-content">
            ...
            <div className="cw-premium-statement-text">
              ...
              <p className="cw-premium-statement-desc">
                Building unforgettable musical experiences through
                collaboration, artistry, innovation, and a shared passion for
                performance.
              </p>
            </div>
