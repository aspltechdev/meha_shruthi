...
const MeetTheVoices = () => {
  ...
  return (
    <section className="mtv-cinematic-wrapper" ref={sectionRef}>
      ...
      {/* ── VIDEO BACKGROUNDS — one per singer, crossfade via opacity ── */}
      <div className="mtv-bg-videos">
        {singers.map((singer, idx) => (
          <video
            key={singer.uniqueId}
            ref={idx === 0 ? videoRefMeera : videoRefKaushik}
            className={`mtv-bg-video ${activeIndex === idx ? 'active' : ''}`}
        ...
        <div className="mtv-bg-scrim" />
        {/* Accent colour tint that shifts per singer */}
        <div
          className="mtv-bg-tint"
          style={{ background: `${currentSinger.accentColor}22` }}
        />
      </div>
      ...
      <div className="mtv-theep-stage">
        {singers.map((singer, idx) => (
          ...
          >
            ...
            {/* ══ RIGHT HALF — single portrait card only ══ */}
