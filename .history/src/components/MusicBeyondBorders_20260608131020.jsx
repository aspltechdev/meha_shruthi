...
const countries = [
  { name: "Canada", image: "/images/countries/canada.jpg", flag: "🇨🇦", city: "Toronto", year: "2019", color: "#e040a0" },
  { name: "United Kingdom", image: "/images/countries/uk.jpg", flag: "🇬🇧", city: "London", year: "2018", color: "#9333ea" },
  { name: "Australia", image: "/images/countries/australia.jpg", flag: "🇦🇺", city: "Sydney", year: "2020", color: "#c084fc" },
  { name: "Denmark", image: "/images/countries/denmark.jpg", flag: "🇩🇰", city: "Copenhagen", year: "2022", color: "#e040a0" },
  { name: "Switzerland", image: "/images/countries/switzerland.jpg", flag: "🇨🇭", city: "Zurich", year: "2021", color: "#9333ea" },
  { name: "Singapore", image: "/images/countries/singapore.jpg", flag: "🇸🇬", city: "Marina Bay", year: "2019", color: "#c084fc" },
  { name: "Malaysia", image: "/images/countries/malaysia.jpg", flag: "🇲🇾", city: "Kuala Lumpur", year: "2023", color: "#e040a0" },
  { name: "Sri Lanka", image: "/images/countries/srilanka.jpg", flag: "🇱🇰", city: "Colombo", year: "2017", color: "#9333ea" },
];
...
const MusicBeyondBorders = () => {
  ...
  const scrollTo = (index) => {
    if (!scrollRef.current) return;
    const container = scrollRef.current;
    const cardWidth = container.scrollWidth / countries.length;
    ...
    <section className="mbb-section">
      ...
      >
        <div className="mbb-scroll-track">
          {countries.map((country, index) => (
            <div
              className={`mbb-card ${index === activeIndex ? 'mbb-card-active' : ''}`}
              key={index}
              style={{ '--card-color': country.color }}
              onClick={() => setActiveIndex(index)}
            >
              {/* Top accent bar */}
              <div className="mbb-card-top-bar"></div>

              {/* Image */}
              <div className="mbb-card-img-wrap">
                <img
                  src={country.image}
                  alt={country.name}
                  className="mbb-card-img"
              ...
              {/* Number */}
              <div className="mbb-card-num">
                {(index + 1).toString().padStart(2, '0')}
              </div>

              {/* Info */}
              <div className="mbb-card-info">
                <h3 className="mbb-card-name">{country.name}</h3>
                <p className="mbb-card-city">📍 {country.city}</p>
                <span className="mbb-card-tag">✈️ International Tour</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Progress dots */}
      <div className="mbb-dots">
        {countries.map((_, i) => (
          <button
            key={i}
            className={`mbb-dot ${i === activeIndex ? 'mbb-dot-active' : ''}`}
            onClick={() => scrollTo(i)}
          />
        ))}
      </div>
      ...
      {/* Progress bar */}
      <div className="mbb-progress-bar">
        <div
          className="mbb-progress-fill"
          style={{ width: `${((activeIndex + 1) / countries.length) * 100}%` }}
        ></div>
      </div>
      <p className="mbb-scroll-hint">← Drag to explore →</p>
      ...
      <div className="mbb-statement">
        ...
        <div className="mbb-statement-line"></div>
        <div className="mbb-flags">
          {countries.map((c, i) => (
            <span key={i} className="mbb-flag-item" style={{ animationDelay: `${i * 0.2}s` }}>
              {c.flag}
            </span>
          ))}
        </div>
