...
import { useEffect, useRef, useState } from "react"; import "./MusicBeyondBorders.css"; const countries = [ { name: "Sri Lanka", image: "/images/countries/srilanka.jpg", flagImage: "https://flagcdn.com/w160/lk.png", city: "Colombo", year: "2017", landmark: "Nelum Pokuna" }, { name: "United Kingdom", image: "/images/countries/uk.jpg", flagImage: "https://flagcdn.com/w160/gb.png", city: "London", year: "2018", landmark: "Royal Albert Hall" }, { name: "Canada", image: "/images/countries/canada.jpg", flagImage: "https://flagcdn.com/w160/ca.png", city: "Toronto", year: "2019", landmark: "CN Tower" }, { name: "Singapore", image: "/images/countries/singapore.jpg", flagImage: "https://flagcdn.com/w160/sg.png", city: "Marina Bay", year: "2019", landmark: "Esplanade" }, { name: "Australia", image: "/images/countries/australia.jpg", flagImage: "https://flagcdn.com/w160/au.png", city: "Sydney", year: "2020", landmark: "Opera House" }, { name: "Switzerland", image: "/images/countries/switzerland.jpg", flagImage: "https://flagcdn.com/w160/ch.png", city: "Zurich", year: "2021", landmark: "Lake Zurich" }, { name: "Denmark", image: "/images/countries/denmark.jpg", flagImage: "https://flagcdn.com/w160/dk.png", city: "Copenhagen", year: "2022", landmark: "Tivoli Gardens" }, { name: "Malaysia", image: "/images/countries/malaysia.jpg", flagImage: "https://flagcdn.com/w160/my.png", city: "Kuala Lumpur", year: "2023", landmark: "Petronas Towers" } ]; const MusicBeyondBorders = () => { const [activeIndex, setActiveIndex] = useState(0); const [paused, setPaused] = useState(false); const globeRef = useRef(null); /* =========================== AUTOPLAY =========================== */ useEffect(() => { if (paused) return; const interval = setInterval(() => { setActiveIndex((prev) => prev === countries.length - 1 ? 0 : prev + 1 ); }, 4000); return () => clearInterval(interval); }, [paused]); /* =========================== GLOBE ROTATION =========================== */ useEffect(() => { if (globeRef.current) { globeRef.current.style.transform = rotate(${activeIndex * 45}deg); } }, [activeIndex]); const activeCountry = countries[activeIndex]; return ( <section className="mbb-section"> {/* BACKGROUND GLOBE */} <div className="mbb-globe-container"> <div className="mbb-globe" ref={globeRef}> {[...Array(6)].map((_, i) => ( <span key={i} className="mbb-globe-line" style={{ transform: rotateY(${i * 30}deg), }} /> ))} {[...Array(4)].map((_, i) => ( <span key={i} className="mbb-globe-parallel" style={{ top: ${25 + i * 15}%, }} /> ))} </div> <div className="mbb-globe-glow" /> </div> {/* HEADER */} <div className="mbb-header"> <div className="mbb-header-badge"> <span className="mbb-badge-dot" /> GLOBAL TOURS <span className="mbb-badge-dot" /> </div> <h2 className="mbb-title"> <span>Music Beyond </span> <span className="accent">Borders</span> </h2> <div className="mbb-metrics"> <div> <strong>8</strong> <span>Countries</span> </div> <div className="divider">•</div> <div> <strong>15+</strong> <span>Cities</span> </div> <div className="divider">•</div> <div> <strong>50+</strong> <span>Shows</span> </div> </div> </div> {/* MAIN CONTENT */} <div className="mbb-main" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} > {/* GLASS CARD */} <div className="mbb-card"> <div className="mbb-card-image"> <img src={activeCountry.image} alt={activeCountry.name} /> </div> <div className="mbb-card-content"> <span className="mbb-card-year"> {activeCountry.year} </span> <h3>{activeCountry.name}</h3> <p> 📍 {activeCountry.city} </p> {/* <div className="mbb-tour-tag"> International Tour </div> */} </div> </div> {/* TIMELINE */} <div className="mbb-timeline"> <div className="mbb-line" /> {countries.map((country, index) => ( <button key={index} className={mbb-point ${ activeIndex === index ? "active" : "" }} onClick={() => setActiveIndex(index) } > {/* <span className="mbb-point-year"> {country.year} </span> */} <span className="mbb-point-flag"> <img src={country.flagImage} alt={country.name} className="mbb-point-flag-img" /> </span> <span className="mbb-point-dot" /> <span className="mbb-point-name"> {country.name} </span> </button> ))} </div> </div> {/* FOOTER STATEMENT */} <div className="mbb-statement"> <div className="mbb-bg-text"> WORLD TOUR </div> <div className="mbb-statement-content"> <div className="mbb-statement-line" /> <h3> Thousands of miles traveled. <br /> <span> Millions of hearts touched. </span> </h3> <div className="mbb-statement-line" /> </div> </div> </section> ); }; export default MusicBeyondBorders; /*